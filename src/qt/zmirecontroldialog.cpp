...
#include "ui_zmirecontroldialog.h"
...
std::list<std::string> ZMireControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZMireControlDialog::listMints;

ZMireControlDialog::ZMireControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZMireControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZMireControlDialog::~ZMireControlDialog()
{
    delete ui;
}

void ZMireControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZMireControlDialog::updateList()
{
...
void ZMireControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZMireControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZMire_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZMireControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZMireControlDialog::GetSelectedMints()
{
...
void ZMireControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
