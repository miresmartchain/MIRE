                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zMIRE ought to be enough for anybody." - Bill Gates, 2017
    ui->zMIREpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzMIRESyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzMIRE->setEnabled(false);
        ui->pushButtonMintzMIRE->setToolTip(tr("zMIRE is currently disabled due to maintenance."));

        ui->pushButtonSpendzMIRE->setEnabled(false);
        ui->pushButtonSpendzMIRE->setToolTip(tr("zMIRE is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zMIREpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzMIRE_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zMIRE is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zMIRE...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zMIRE in ") +
...
void PrivacyDialog::on_pushButtonSpendzMIRE_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zMIRE is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zMIRE
        sendzMIRE();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zMIRE
    sendzMIRE();
}

void PrivacyDialog::on_pushButtonZMireControl_clicked()
{
    ZMireControlDialog* zMireControl = new ZMireControlDialog(this);
    zMireControl->setModel(walletModel);
    zMireControl->exec();
}

void PrivacyDialog::setZMireControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzMireSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzMIRE()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Mire Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zMIREpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zMIRE is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " MIRE </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zMIRE</b>";
    ...
    // use mints from zMire selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZMireControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZMireControlDialog::GetSelectedMints();
    }
    ...
    // Clear zmire selector in case it was used
    ZMireControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zMire Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Mire, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zMire Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zMIRE </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zMIRE "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zMIRE "));
    ui->labelzMIREAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzMIRESyncStatus->setVisible(fShow);
}
