...
#if defined(NDEBUG)
#error "Mire cannot be compiled without assertions."
#endif
...
unsigned int nStakeMinAge = 1 * 60 * 60; //1 hours RELEASE
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zMire spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zMire spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zMire spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
   ...
   if(nHeight == 1){
        nSubsidy =  30000000  * COIN; //premine 30 Million MIRE
    }  
	else if(nHeight < 1440){
        nSubsidy =  0.0007  * COIN; //POW Block END For Runing Chain
    }
	else if(nHeight < 86400){
        nSubsidy =  0.00079822  * COIN; //POS Block Phase 0
    }
	else if(nHeight < 604801){
        nSubsidy =  30 * COIN; //Phase 1
    }
	else if(nHeight < 1123202){
        nSubsidy =  35  * COIN; //Phase 2
    }
	else if(nHeight < 1641603){
        nSubsidy =  40  * COIN; //Phase 3
    }
	else if(nHeight < 2160004){
        nSubsidy =  50  * COIN; //Phase 4
    }
	else if(nHeight < 2678405){
        nSubsidy =  55  * COIN; //Phase 5
    }
	else if(nHeight < 3196806){
        nSubsidy =  60  * COIN; //Phase 6
    }
	else if(nHeight < 3715207){
        nSubsidy =  70  * COIN; //Phase 7
    }
	else if(nHeight < 4233608){
        nSubsidy =  50  * COIN; //Phase 8
    }
	else if(nHeight < 4752009){
        nSubsidy =  30  * COIN; //Phase 9
    }
	else if(nHeight < 5270410){
        nSubsidy =  20  * COIN; //Phase 10
    }
	else if(nHeight < 5788811){
        nSubsidy =  10  * COIN; //Phase 11
    }
	else if(nHeight < 6307212){
        nSubsidy =  9  * COIN; //Phase 12
    }
	else if(nHeight < 6825613){
        nSubsidy =  8  * COIN; //Phase 13
    }
	else if(nHeight < 7344014){
        nSubsidy =  7  * COIN; //Phase 14
    }
	else if(nHeight < 7862415){
        nSubsidy =  6  * COIN; //Phase 15
    }
	else if(nHeight < 10454420){
        nSubsidy =  5  * COIN; //Phase 16,Phase 17,Phase 18,Phase 19,Phase 20
    }
	else if(nHeight < 10972821){
        nSubsidy =  3  * COIN; //Phase 21
    } else {
		
		nSubsidy =  0.01  * COIN; //Phase Z
	}
    return nSubsidy;
}
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
	...
	// 80% for Masternodes 3% Devloper Fee
    if (nHeight <= Params().LAST_POW_BLOCK()) {
	      ret = blockValue  / 100 * 0;
	} else if (nHeight > 1) {
          ret = blockValue  / 100 * 80; //80%, 5% - def fee reward
...
CBitcoinAddress addressExp1("MXjPF8HRPan6yPEyuoHcNi98fPCzhzXMhL");
CBitcoinAddress addressExp2("MZRTs7T3RNQN9AVcrxQuR2fPzonZHRKyAV");
...
if (pindex->GetBlockHash() != view.GetBestBlock())
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Mire
...
void RecalculateZMIRESpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zMIRE supply
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zMire with serial %s is already in the block %d\n",
   ...
   if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateMIRESupply(Params().Zerocoin_StartHeight());
    }
    ...
    //LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zMireSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert MIRE to zMIRE
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zMire spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zMire serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zMire mints and %d zMire spends\n", __func__, nMints, nSpends);
...
bool LoadExternalBlockFile(FILE* fileIn, CDiskBlockPos* dbp)
{
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                // locate a header
                unsigned char buf[MESSAGE_STMIRE_SIZE];
                blkdat.FindByte(Params().MessageStart()[0]);
                nRewind = blkdat.GetPos() + 1;
                blkdat >> FLATDATA(buf);
                if (memcmp(buf, Params().MessageStart(), MESSAGE_STMIRE_SIZE))
                    continue;
                // read size
                blkdat >> nSize;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Mire: We use certain sporks during IBD, so check to see if they are
...
bool ProcessMessages(CNode* pfrom)
{
    ...
    while (!pfrom->fDisconnect && it != pfrom->vRecvMsg.end()) {
        ...
        if (memcmp(msg.hdr.pchMessageStart, Params().MessageStart(), MESSAGE_STMIRE_SIZE) != 0) {
            LogPrintf("PROCESSMESSAGE: INVALID MESSAGESTMIRE %s peer=%d\n", SanitizeString(msg.hdr.GetCommand()), pfrom->id);
            fOk = false;
            break;
        }
